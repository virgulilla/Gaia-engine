...
namespace GaiaEngine.Simulation.Tests.Interactions.Feeding;

public sealed class DeterministicFeedingSystemTests
{
    [Fact]
    public void Execute_ShouldConsumeVegetationAndReduceHunger()
    {
        GaiaEngine.Domain.World.World world = CreateWorld(vegetationAmount: 400);
        OrganismCollection organisms = CreateOrganisms();
        DeterministicFeedingSystem system = new();
        FeedingRequestCollection requests = new(
            new[]
            {
                new FeedingRequest(
        ...
        FeedingSystemResult result = system.Execute(world, organisms, requests);

        Assert.Empty(result.RemainingRequests.GetAll());
        Assert.Equal(360, result.Organisms.Get(OrganismId.FromSequence(new EntitySequence(100))).Needs.Hunger);
        Assert.True(result.World.GetChunk(new ChunkCoordinates(0, 0)).Resources.TryGet(ResourceType.Vegetation, out ResourceState? vegetation));
        Assert.NotNull(vegetation);
        Assert.Equal(388, vegetation!.CurrentAmount);
    }
    ...
    public void Execute_ShouldKeepFutureRequestsPending()
    {
        ...
        Assert.Equal(600, result.Organisms.Get(OrganismId.FromSequence(new EntitySequence(100))).Needs.Hunger);
    }

    [Fact]
    public void Execute_ShouldIgnoreRequestsWithoutEnoughVegetation()
    {
        GaiaEngine.Domain.World.World world = CreateWorld(vegetationAmount: 5);
        OrganismCollection organisms = CreateOrganisms();
        DeterministicFeedingSystem system = new();
        FeedingRequestCollection requests = new(
            new[]
        ...
        Assert.True(result.World.GetChunk(new ChunkCoordinates(0, 0)).Resources.TryGet(ResourceType.Vegetation, out ResourceState? vegetation));
        Assert.NotNull(vegetation);
        Assert.Equal(5, vegetation!.CurrentAmount);
    }

    private static GaiaEngine.Domain.World.World CreateWorld(int vegetationAmount)
    {
            ...
            new[]
            {
                new Chunk(
                    ...
                    new ChunkResources(
                        new ResourceState[]
                        {
                            new(ResourceId.FromSequence(new EntitySequence(21)), ResourceType.Vegetation, ResourceCategory.Renewable, vegetationAmount, 500, 4, 70, 800),
        ...
        return new OrganismCollection(
            new[]
            {
                new Organism(
                    ...
                    new NeedsComponent(600, 100, 100, 0),
